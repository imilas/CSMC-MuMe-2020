...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newcommand{\descfirst}{\textit{Descision.1~}}
\newcommand{\descsecond}{\textit{Descision.2}}
...
TPEs combine different stand-alone models specializing in~\descfirst or~\descsecond~and are trained only with our FFT features. To train TPEs for \descfirst, we use all drums in RadarDB and FreeDB (categories do not matter). For the~\descsecond, we combine the two databases and merge toms into kicks. Rims and shakers are merged with FreeDB's \enquote{other} category. MEMs are trained with autoencoder embeddings and envelope features and give simultaneous answers to both decisions. MEMs are trained with with the kicks, snares, claps and hat sounds from RadarDB and FreeDB.


%  Our best TPE models achieve 82\% accuracy in a 6-way drum categorization task and 98\% accuracy in separation of drums from non-drum sounds. Our best MEM achieves 91\% accuracy in a 4 way drum categorization task and 99\% accuracy in separation of drums from non-drums. Detailed measurements can be found in Appendix~\ref{appendix:classifier_definitions}. 
...
\begin{comment}
% AH: THIS TABLE IS COMMENTED OUT
\begin{table}[h!]
...
% We again measure this coefficient after dropping samples that were categorized as \enquote{bad} by one or both authors, and after after dropping all \enquote{bad} and \enquote{other} samples.
...
% Suggesting that latent representations of autoencoder networks can be used as general,low-dimensional proxies for representation of complex inputs.
